...
export default async function httpRequest(options, expectStatusCode = 200) {
  try {
    ...
    console.log(`${logTime(options.reqId)} HTTP ${options.method} request sent to '${options.url}'`);
    const response = await got(options);
    // Validate response code
    if (expectStatusCode.toString().indexOf(response.statusCode.toString()) !== -1) {
      // Response good
      console.log(`${logTime(options.reqId)} HTTP ${options.method} request received from '${options.url}' with status code '${response.statusCode}'`);
      return response;
    }
    // Response bad
    ...
    // Error
    console.error(`${logTime(options.reqId)} HTTP request error...`, error.message);
    return false;
  }
}
